...
<p class="manage"><a href="<?php echo zen_href_link(FILENAME_WISHLISTS, '', 'SSL'); ?>"><?php echo UN_TEXT_MANAGE_WISHLISTS; ?></a></p>
	<?php if ( UN_ALLOW_MULTIPLE_WISHLISTS ) { ?>
	<p class="manage"><a href="<?php echo zen_href_link(FILENAME_WISHLIST_MOVE, 'wid='.$id, 'SSL'); ?>"><?php echo UN_TEXT_WISHLIST_MOVE; ?></a></p>
	<?php } ?>
...
?>
    ...
    $display_qty = (($flag_show_product_info_in_cart_qty == 1) ? '<p>' . PRODUCTS_ORDER_QTY_TEXT_IN_CART . '1' . '</p>' : '');
            if (($products->fields['products_qty_box_status'] == 0 || $products->fields['products_quantity_order_max']== 1)) {
    ...
    $the_button = PRODUCTS_ORDER_QTY_TEXT . '<input type="text" name="cart_quantity" value="'. $products->fields['quantity'] . '" size="2" maxlength="3" class="xs" /><br />' . zen_get_products_quantity_min_units_display($products->fields['products_id']) . '<br />' . zen_draw_hidden_field('products_id', $products->fields['products_id']) . zen_image_submit(BUTTON_IMAGE_IN_CART, BUTTON_IN_CART_ALT);
            }
